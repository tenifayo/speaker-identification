        ...
        with self._get_connection() as conn:
            ...
            return [
                AccessLog(
                    ...
                    sentence_match=bool(row['sentence_match']) if row.get('sentence_match') is not None else None,
                    liveness_passed=bool(row['liveness_passed']) if row.get('liveness_passed') is not None else None
                )
                for row in rows
            ]
